...
`PROPOSITION-PHASE-2.md` contient le chiffrage, les marges et le tarif de lancement : il reste
dans `.gitignore` tant que le dépôt est public. Même vigilance pour tout document commercial à
venir.
...
```bash
npm run build              # l'export statique se construit
node scripts/audit.mjs     # 39 contrôles, serveur de dev requis
```
